...
import {
  QuickbooksBadRequestException,
  QuickbooksUnauthorizedException,
} from "./exceptions";
...
export class BaseService<Response, Query, QueryResponse> {
  ...
  public query(
    condition: WhereOptions<Query>,
    options?: QueryOptions
  ): Observable<QueryResponse> {
    return this.getHttpHeaders()
      .pipe(
        mergeMap((headers) =>
          this.http.get<QueryResponse>(
            this.queryUrl(QueryStatementType.Select, condition, options),
            {
              headers,
              params: {
                ...this.minorVersion,
  ...
  public count(condition: WhereOptions<Query>): Observable<QueryResponse> {
    return this.getHttpHeaders()
      .pipe(
        mergeMap((headers) =>
          this.http.get<QueryResponse>(
            this.queryUrl(QueryStatementType.Count, condition),
            {
              headers,
              params: {
                ...this.minorVersion,
  ...
  protected post<R = Response>(
    ...
    return this.getHttpHeaders()
      .pipe(
        mergeMap((authHeaders) =>
          this.http.post<R>(this.url(path), body, {
            headers: {
              ...authHeaders,
              ...headers,
            },
  ...
  protected queryUrl(
    statement: QueryStatementType,
    condition: WhereOptions<any>,
    options?: QueryOptions
  ): string {
    return `${this.apiUrl}/v3/company/${
      this.realm
    }/query?${QueryUtils.generateQuery(
      this.resource,
      statement,
      condition,
      options
